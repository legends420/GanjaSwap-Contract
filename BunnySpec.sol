...
// File: @seedswap/seed-swap-lib/contracts/token/BEP20/IBEP20.sol
...
// File: contracts/SeedBunnies.sol
...
contract SeedBunnies is ERC721, Ownable {
    ...
    constructor(string memory _baseURI) public ERC721("Seed Bunnies", "PB") {
...
contract BunnyMintingStation is AccessControl {
    ...
    constructor(SeedBunnies _seedBunnies) public {
        seedBunnies = _seedBunnies;
        _setupRole(DEFAULT_ADMIN_ROLE, _msgSender());
    }

    /**
     * @dev Mint NFTs from the SeedBunnies contract.
     * Users can specify what bunnyId they want to mint. Users can claim once.
     * There is a limit on how many are distributed. It requires SEED balance to be > 0.
     */
...
// File: contracts/utils/SeedProfile.sol
...
contract SeedProfile is AccessControl, ERC721Holder {
     ...
     * and sends SEED to burn address. Requires 2 token approvals.
     */
    function createProfile(
        ...
        // Transfer SEED tokens to this contract
        seedToken.safeTransferFrom(
            _msgSender(),
            address(this),
    ...
    function updateProfile(address _nftAddress, uint256 _tokenId) external {
        ...
        // Transfer SEED token to this address
        seedToken.safeTransferFrom(
            _msgSender(),
            address(this),
            numberCakeToUpdate
    ...
    /**
     * @dev Claim SEED to burn later.
     * Callable only by owner admins.
     */
    function claimFee(uint256 _amount) external onlyOwner {
        seedToken.safeTransfer(_msgSender(), _amount);
    }
    ...
    /**
     * @dev Update the number of SEED to register
     * Callable only by owner admins.
     */
...
contract BunnySpecialV1 is Ownable {
    ...
    SeedProfile public seedProfile;
    ...
    function mintNFT(uint8 _bunnyId) external {
        ...
        (userId, , , , , isUserActive) = seedProfile.getUserProfile(
            senderAddress
        );
        ...
        if (bunnyCharacteristics[_bunnyId].seedCost > 0) {
            seedToken.safeTransferFrom(
                senderAddress,
                address(this),
                bunnyCharacteristics[_bunnyId].seedCost
            );
        }
    ...
    function addBunny(
        ...
        emit BunnyAdd(_bunnyId, _thresholdUser, _seedCost);
    }

    /**
     * @dev It transfers the SEED tokens back to the chef address.
    ...
    function updateBunny(
        ...
        bunnyCharacteristics[_bunnyId].seedCost = _seedCost;
        bunnyCharacteristics[_bunnyId].isActive = _isActive;

        emit BunnyChange(_bunnyId, _thresholdUser, _seedCost, _isActive);
    }
    ...
    function canClaimSingle(address _userAddress, uint8 _bunnyId)
        ...
        returns (bool)
    {
        if (!seedProfile.hasRegistered(_userAddress)) {
            return false;
        } else {
            uint256 userId;
            bool userStatus;

            (userId, , , , , userStatus) = seedProfile.getUserProfile(
                _userAddress
            );
