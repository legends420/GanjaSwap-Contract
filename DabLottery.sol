...
// File: contracts/interfaces/IShatterLottery.sol


pragma solidity ^0.8.4;

interface IShatterLottery {
    /**
    ...
    /**
     * @notice Draw the final number, calculate reward in DAB per group, and make lottery claimable
     * @param _lotteryId: lottery id
     * @dev Callable by operator
     */
     ...
     * @notice Inject funds
     * @param _lotteryId: lottery id
     * @param _amount: amount to inject in DAB token
     * @dev Callable by operator
     */
     ...
     * @param _priceTicketInDab: price of a ticket in DAB
...
// File: contracts/ShatterLottery.sol


pragma solidity ^0.8.4;
pragma abicoder v2;

/** @title Shatter Lottery.
 * @notice It is a new contract for lottery.
 */
contract ShatterLottery is ReentrancyGuard, IShatterLottery, Ownable {
    ...
    uint256 public constant MIN_LENGTH_LOTTERY = 700;
    uint256 public constant MAX_LENGTH_LOTTERY = 100000;
    ...
    struct Lottery {
        ...
        uint256 startTime;
        uint256 endTime;
        uint256 priceTicketInDab;
        uint256 discountDivisor;
        uint256[6] rewardsBreakdown; // 0: 1 matching number // 5: 6 matching numbers
        uint256 treasuryFee; // 500: 5% // 200: 2% // 50: 0.5%
        uint256[6] dabPerBracket;
        uint256[6] countWinnersPerBracket;
        uint256 firstTicketId;
        uint256 firstTicketIdNextLottery;
        uint256 amountCollectedInDab;
        uint32 finalNumber;
    }
    ...
    event LotteryInjection(uint256 indexed lotteryId, uint256 amount);
    event LotteryOpen(
     ...
     * @param _dabTokenAddress: address of the DAB token
     * @param _randomGeneratorAddress: address of the RandomGenerator contract used to work with ChainLink VRF
     */
    constructor(address _dabTokenAddress, address _randomGeneratorAddress) {
        dabToken = IERC20(_dabTokenAddress);
        ...
        nonReentrant
    {
        ...
        // Calculate number of DAB to this contract
        uint256 amountDabToTransfer =
            _calculateTotalPriceForBulkTickets(
                _lotteries[_lotteryId].discountDivisor,
                _lotteries[_lotteryId].priceTicketInDab,
                _ticketNumbers.length
            );

        // Transfer dab tokens to this contract
        dabToken.safeTransferFrom(address(msg.sender), address(this), amountDabToTransfer);

        // Increment the total amount collected for the lottery round
        _lotteries[_lotteryId].amountCollectedInDab += amountDabToTransfer;
    ...
    function claimTickets(
        ...
        // Initializes the rewardInDabToTransfer
        uint256 rewardInDabToTransfer;
        ...
        for (uint256 i = 0; i < _ticketIds.length; i++) {
            ...
            rewardInDabToTransfer += rewardForTicketId;
        }

        // Transfer money to msg.sender
        dabToken.transfer(msg.sender, rewardInDabToTransfer);

        emit TicketsClaim(msg.sender, rewardInDabToTransfer, _lotteryId, _ticketIds.length);
    }
    ...
    function drawFinalNumberAndMakeLotteryClaimable(uint256 _lotteryId) external override onlyOperator nonReentrant {
        ...
        uint256 amountToShareToWinners =
            (((_lotteries[_lotteryId].amountCollectedInDab) * (10000 - _lotteries[_lotteryId].treasuryFee))) / 10000;

        // Initializes the amount to withdraw to treasury
        uint256 amountToWithdrawToTreasury;

        // Calculate prizes in DAB for each bracket by starting from the highest one
        for (uint32 i = 0; i < 6; i++) {
            ...
            if (
                ...
                if (_lotteries[_lotteryId].rewardsBreakdown[j] != 0) {
                    _lotteries[_lotteryId].dabPerBracket[j] =
                ...
                // A. No DAB to distribute, they are added to the amount to withdraw to treasury address
            } else {
                _lotteries[_lotteryId].dabPerBracket[j] = 0;
        ...
        amountToWithdrawToTreasury += (_lotteries[_lotteryId].amountCollectedInDab - amountToShareToWinners);

        // Transfer DAB to treasury address
        dabToken.safeTransfer(treasuryAddress, amountToWithdrawToTreasury);
    ...
    function changeRandomGenerator(address _randomGeneratorAddress) external onlyOwner {
        ...
        // Calculate the finalNumber based on the randomResult generated by ChainLink's fallback
        uint32 finalNumber = IRandomNumberGenerator(_randomGeneratorAddress).viewRandomResult();
    ...
    function injectFunds(uint256 _lotteryId, uint256 _amount) external override onlyOwner {
        ...
        dabToken.safeTransferFrom(address(msg.sender), address(this), _amount);
        _lotteries[_lotteryId].amountCollectedInDab += _amount;
...
˙        );
        ...
        _lotteries[currentLotteryId] = Lottery({
            ...
            endTime: _endTime,
            priceTicketInDab: _priceTicketInDab,
            discountDivisor: _discountDivisor,
            rewardsBreakdown: _rewardsBreakdown,
            treasuryFee: _treasuryFee,
            dabPerBracket: [uint256(0), uint256(0), uint256(0), uint256(0), uint256(0), uint256(0)],
            countWinnersPerBracket: [uint256(0), uint256(0), uint256(0), uint256(0), uint256(0), uint256(0)],
            firstTicketId: currentTicketId,
            firstTicketIdNextLottery: currentTicketId,
            amountCollectedInDab: 0,
            finalNumber: 0
        });

        emit LotteryOpen(currentLotteryId, block.timestamp, _endTime, _priceTicketInDab, currentTicketId);
    }
    ...
    function recoverWrongTokens(address _tokenAddress, uint256 _tokenAmount) external onlyOwner {
        require(_tokenAddress != address(dabToken), "Cannot be DAB token");
     ...
     * @param _priceTicket price of a ticket (in DAB)
    ...
    function _calculateRewardsForTicketId(
        ...
        if (transformedWinningNumber == transformedUserNumber) {
            return _lotteries[_lotteryId].dabPerBracket[_bracket];
        } else {
            return 0;
        }
    }
